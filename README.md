...
### A note on AI/ML — read this before assuming otherwise

This prototype is a **basic functional outline**, and in its current form it does **not run live AI/ML models**:

- The **Financial Health Score** and **Multi-Goal Planner** are deterministic math — real, correct, and tested, but not AI.
- The **Fraud Shield** is wired to call a real Hugging Face ML classifier, but automatically falls back to a transparent, rule-based keyword scorer whenever the ML model can't be loaded (e.g. no internet access to huggingface.co in a given environment). Check the caption shown under each result in the app — it tells you honestly which one just ran.
- The **AI mentor** (LLM-based conversational layer) described in the project proposal is **not implemented in this prototype at all**.

The full AI/ML-powered vision — a live ML fraud classifier and an LLM-based trilingual mentor — is completely designed in the project proposal, with the exact models, prompt design, and architecture specified. Wiring those in live is the next development phase, not further design work. This distinction is stated the same way in the application form, so there's no gap between what's claimed there and what this code actually does.
